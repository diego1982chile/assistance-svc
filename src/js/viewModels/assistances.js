...
define(['knockout',       
 ...
 function(ko, DemoDelayingDataProvider, ojconverterutils_i18n_1, AsyncRegExpValidator, ArrayDataProvider, ListDataProviderView, ojdataprovider_1) {
     
    function AssistancesViewModel() {
        ...
        self.date = ko.observable(ojconverterutils_i18n_1.IntlConverterUtils.dateToLocalIso(new Date().setMilliseconds(0)));

        //Labelled Buttonset
        self.assistanceOptions = [            
            { id: "yesopt", value: true, assistance: "Yes" },
            { id: "noopt", value: false, assistance: "No" },
        ];
        ...
        self.datasource = ko.computed(function () {                        
            ...
            $.getJSON(ko.dataFor(document.getElementById('globalBody')).assistanceServiceBaseUrl() + "events/date/" + self.date()).
                then(function (events) {                                      
                    events.forEach(function(event) {                    
                        var tokens = event.begin.split(":");
                        event.begin = tokens[0] + ":" + tokens[1];
                        var tokens = event.end.split(":");
                        event.end = tokens[0] + ":" + tokens[1];                        
                    });                                      
                    self.data(events);                                        
            });             
            ...
            return new DemoDelayingDataProvider(new ListDataProviderView(arrayDataProvider, 
        ...
        self.formatDay = (date) => {
            let formattedDay = {disabled:false};
            let day;
            let month;
            ...
            let dateStr = date.fullYear+'-'+month+'-'+day+'T00:00:00';
            let newDate = new Date(dateStr)
            let dayofWeek = newDate.getDay();
            console.log('dayofWeek: ' + dayofWeek);            
            
            if(dayofWeek === 0 || dayofWeek === 6) {
              formattedDay = {disabled:true};
            }
            return formattedDay
        }

        self.getStringFromHour = function(date, format) {
